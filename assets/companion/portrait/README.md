...
另加入使用者提供的 13 張 1254×1254 RGBA PNG 原圖：`home/` 六張、`outdoor/` 七張，均保持原像素。
三套服裝分別為一般 `normal`（原本八張）、居家 `home`、外出 `outdoor`。
桌寵對話輸入「換居家服」、「換外出服」、「換一般服」直接切換；「換衣服」依一般 → 居家 → 外出循環；服裝選擇存在本機。
居家沒有閉眼／說話圖時使用居家待機圖；開心／驚訝使用居家害羞圖。
外出沒有難過／睡覺圖時使用外出害羞／閉眼圖。缺圖時不會套用別套服裝。
居家坐姿與外出站姿佔用畫面的比例不同，素材保持原比例，未強行裁切或拉伸。
